...
public static class FunctionLibrary
{
    ...
    {
        Wave, MulitWave, Ripple, Sphere, Torus
    }
    ...
    public static Vector3 MultiWave(float u, float v, float t) {
        ...
        p.x = u;
        p.y = Sin(PI * (u + 0.5f * t));
        p.y += Sin(2f * PI * (v + u + t)) * 0.5f;
        p.y += Sin(PI * (u + v + 0.25f * t));
        p.y *= (1f / 2.5f); //scale function down to fit (-1,1) range 
        p.z = v;
        return p; //scale function down to fit (-1,1) range 
    }
